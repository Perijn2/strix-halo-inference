...
# The command reads deployment settings without evaluating .env, so Compose-escaped bcrypt hashes remain safe.
set -euo pipefail
...
ornith_model_dir="$(awk -F= '/^ORNITH_MODEL_DIR=/ { print substr($0, index($0, "=") + 1); exit }' .env)"
[[ -n "$ornith_model_dir" ]] || { printf '%s\n' 'ORNITH_MODEL_DIR is required in .env.'; exit 1; }
test -x "$ornith_model_dir/bundle/serve.sh" || { printf 'Missing Ciru launcher: %s/bundle/serve.sh\n' "$ornith_model_dir"; exit 1; }
